...
implement:
  ...
  for_each_part:
    ...
    step_0b: |
      If part has UI → COPY FIGMA STRUCTURE before anything else (literal-pipeline-figma-coding-rules STRUCTURE_COPY_RULE):
    ...
    step_1: "If part has UI/CSS → run Figma CSS Extract (literal-pipeline-figma-coding-rules section 1) to get exact CSS values for the skeleton from step_0b"
    step_2: "Implement code: combine HTML skeleton (step_0b) + CSS values (step_1) + existing pattern (step_0)"
    step_3: "If part has UI/CSS → run Figma Self-Verify (literal-pipeline-figma-coding-rules section 2) — verify BOTH structure AND CSS properties against Figma"
    ...
    commit_gate: |
      ...
      If ANY check fails → run Self-Verify (literal-pipeline-figma-coding-rules section 2) BEFORE committing.
  ...
  L_XL_research:
    ...
    action: "Dispatch literal-pipeline-code-researcher via Agent tool"
...
```yaml
handoff:
  to: "literal-pipeline-code-reviewer"
  required_fields:
...
```yaml
figma_rules:
  PREREQUISITE: "If design adapter active → load Skill: literal-pipeline-figma-coding-rules before implementing any part"
...
css_architecture:
  ...
  workflow:
    step_1: "Extract exact values from Figma (literal-pipeline-figma-coding-rules section 1)"
